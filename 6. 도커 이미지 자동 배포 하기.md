...
```Shell
$ docker run -u root --rm -p 8080:8080 --name jenkins_home -v /home/jenkins_home:/var/jenkins_home -v /var/run/docker.sock:/var/run/docker.sock ywjung99/jenkins:v3
...
1. http://localhost8080접속 후 로그에 적힌 패스워드를 입력합니다.
...
4. Instance Configuration에서 `Save and Finish`를 클릭하면 완료 된다. 그후 http://localhost8080으로 재접속 한다.
